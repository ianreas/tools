...
import { increment, writeBatch, doc, getFirestore, setDoc, updateDoc } from "firebase/firestore";
...
export default function ImageUploader({postRef}) {
  ...
  const [thepictureCode, setThePictureCode] = useState('')

  const [post] = useDocumentDataOnce(postRef)


  const updateLink = async(url) => {
    await updateDoc(postRef, {pictureCode: url})
  }
